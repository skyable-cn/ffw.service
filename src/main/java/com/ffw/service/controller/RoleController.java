...
public class RoleController extends BaseController {
	...
	@RequestMapping(value = "listAllModule", method = RequestMethod.POST)
	public List<PageData> listAllModule(@RequestBody PageData pd) {
		List<PageData> data = null;
		try {
			data = roleService.listAllModule(pd);
		} catch (Exception e) {
	...
	public Page listPage(@RequestBody PageData pd) {
		...
		List<PageData> data = null;
		try {
			data = roleService.listPage(page);
		} catch (Exception e) {
			data = new ArrayList<PageData>();
			e.printStackTrace();
		}
