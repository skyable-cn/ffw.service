...
@RestController
@RequestMapping("orderuse")
public class OrderuseController extends BaseController {

	@Autowired
	IOrderuseService orderuseService;

	@RequestMapping(value = "listAll", method = RequestMethod.POST)
	public List<PageData> listAll(@RequestBody PageData pd) {
		List<PageData> data = null;
		try {
			data = orderuseService.listAll(pd);
		} catch (Exception e) {
	...
	public Page listPage(@RequestBody PageData pd) {
		...
		List<PageData> data = null;
		try {
			data = orderuseService.listPage(page);
		} catch (Exception e) {
			data = new ArrayList<PageData>();
			e.printStackTrace();
		}
	...
	public PageData find(@RequestBody PageData pd) {
		try {
			pd = orderuseService.findById(pd);
		} catch (Exception e) {
			pd = null;
			e.printStackTrace();
		}
