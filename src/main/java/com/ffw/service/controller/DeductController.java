...
public class DeductController extends BaseController {
	...
	public Page listPage(@RequestBody PageData pd) {
		...
		List<PageData> data = null;
		try {
			data = deductService.listPage(page);
		} catch (Exception e) {
			data = new ArrayList<PageData>();
			e.printStackTrace();
		}
