...
public class SupermaninfoController extends BaseController {
	...
	public Page listPage(@RequestBody PageData pd) {
		...
		List<PageData> data = null;
		try {
			data = supermaninfoService.listPage(page);
		} catch (Exception e) {
			data = new ArrayList<PageData>();
			e.printStackTrace();
		}
	...
	public PageData find(@RequestBody PageData pd) {
		try {
			pd = supermaninfoService.findById(pd);
		} catch (Exception e) {
			pd = null;
			e.printStackTrace();
		}
	...
	public PageData findBy(@RequestBody PageData pd) {
		try {
			pd = supermaninfoService.findBy(pd);
		} catch (Exception e) {
			pd = null;
			e.printStackTrace();
		}
